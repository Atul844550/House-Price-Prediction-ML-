...
if st.sidebar.button("🚀 Predict Price"):
    if all(value == 0 for value in input_data):
        st.warning("⚠️ Please enter valid values. All fields cannot be zero!")
    else:
        predicted_price = predict_price(input_data)
        st.markdown(
            f"<p class='big-font' style='color: #4CAF50;'>🏠 Predicted Median House Price: <br> "
            f"<span style='font-size:32px;'>${predicted_price * 100000:.2f}</span></p>", 
            unsafe_allow_html=True
        )
